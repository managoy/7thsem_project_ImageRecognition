...
public class detectaricraft {
    ...
    public detectaricraft(){
         for (int i = 0; i < positives.length; i++) {
            
            
            positives[i] = imread("aircraft/air"+i+".jpg",Imgcodecs.CV_LOAD_IMAGE_GRAYSCALE);
            Imgproc.resize(positives[i],positives[i],new Size(128,64));
        }
        
        
        for (int i = 0; i < negatives.length; i++) {
            negatives[i] = imread("aircraft/n"+i+".jpg",Imgcodecs.CV_LOAD_IMAGE_GRAYSCALE);
             Imgproc.resize(negatives[i],negatives[i],new Size(128,64));
        }
//Mat pos=imread("aircraft/air0.jpg",Imgcodecs.CV_LOAD_IMAGE_GRAYSCALE);
//Imgproc.resize(pos,pos,new Size(128,64));
        
    hogDesc.compute(positives[0],descriptors);
...
//        
      svm.save("finalaircraft.yml");
//SVM svm= SVM.load("finalair.yml");
      ...
      for(int i=0;i<1;i++){
          ...
          String b1 =((predictions.get(i, 0)[0]<0.0)? "Not an Aeroplane":"Is an Aeroplane");
